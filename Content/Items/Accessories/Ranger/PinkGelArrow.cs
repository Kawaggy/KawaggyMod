...
{
    public class PinkGelArrow : KItem
    {
        public override string Texture => Assets.Items.Accessories + "Ranger/Arrow";
        private readonly string slimeTexture = Assets.Items.Accessories + "Ranger/PinkGelArrow_Overlay";

        public override bool Autoload(ref string name)
        {
            KawaggyPlayerEvents.ModifyHitNPCWithProjEvent += SpawnBouncyArrow;
        ...
        {
            player.Kawaggy().pinkGelArrow = true;
        }

        private const float chance = 0.25f;
        private void SpawnBouncyArrow(Player player, Projectile proj, NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
        {
            if (player.Kawaggy().pinkGelArrow)
            {
                if (proj.arrow)
                {
                    if (proj.type != ModContent.ProjectileType<PinkGelArrowProj>())
                    {
                        if (Main.rand.NextFloat() < chance)
                        {
                            Vector2 velocity = Vector2.Normalize(target.Center - player.Center) * 10f;
                            int type = ModContent.ProjectileType<PinkGelArrowProj>();
                            Projectile.NewProjectile(player.position, velocity, type, proj.damage.RandomDamage(0.5f), 0f, player.whoAmI, 5);
                        }
                    }
                }
            }
